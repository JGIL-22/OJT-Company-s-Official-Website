...
// Exposed for inline HTML calls (if any remain)
window.toggleMobileMenu = toggleMobileMenu;
window.toggleChat = () => toggleChat(); // toggleChat is assigned later, so wrap it
...
// Exposed for inline HTML calls (if any remain)
// Note: toggleMobileMenu is safe to expose as it handles its own null checks
// toggleChat is defined inside initChatWidget so we can't expose it directly here unless we refactor scope or expose from inside.
// However, initChatWidget assigns it to the module-level 'toggleChat'.
// To expose it to window, we need to do it after assignment or change strategy.
// Actually, 'toggleChat' is defined as let in root scope.
